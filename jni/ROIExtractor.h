...
public:
	...
	struct setStruct {
		...
		int dataFill(CvPoint p1) {
			...
			if (distX > 125 && distX <= 240 && distY < 25 && this->X1 < distX) {
				this->X1 = distX;
				meetY = 0;
			} else if (distY > 330 && distY < 450 && distX < 25
					&& this->Y1 < distY) {
				this->Y1 = distY;
				meetX = 0;
			} else if (distY > 330 && distY < 450 && distX > 125
					&& distX <= 240) {
				this->Y1 = distY;
				this->X1 = distX;
			} else {
	...
	void mergeSet(int X, int Y,
			vector<pair<int, pair<int, setStruct> > > &contains, CvPoint p1,
			CvPoint p2);
	...
	bool find_if_Contain(vector<pairing>::iterator first,
