...
bool ROIExtractor::find_if_Contain(vector<pairing>::iterator first,
        vector<pairing>::iterator last, int val) {
...
void ROIExtractor::mergeSet(int X, int Y,
        vector<pair<int, pair<int, setStruct > > >&contains, CvPoint p1, CvPoint p2) {
...
bool isOnTheSameline(CvPoint p1, CvPoint p2, vector<CvPoint*> &lineContainer) {
    ...
    for (int i = 0; i < lineContainer.size(); i++) {
        ...
        if (blockY < 25 && blockX < 120)
            continue;
        if (points[0].x == -1 && points[0].y == -1 && points[1].x == -1
                && points[1].y == -1)
            continue;
...
bool sortOutPointX(pair<CvPoint, int> &p1, pair<CvPoint, int> &p2) {
    return p1.first.x == p2.first.x ?
            p1.first.y < p2.first.y : p1.first.x < p2.first.x;
}

bool sortOutPointY(pair<CvPoint, int> &p1, pair<CvPoint, int> &p2) {
    return p1.first.y == p2.first.y ?
            p1.first.x < p2.first.x : p1.first.y < p2.first.y;
}
...
vector<vector<pair<CvPoint, int> > > ROIExtractor::imageROIExtractor(
    ...
    for (i = 0; i < cornerContainer.size(); i++) {
        ...
        tempContainer.push_back(
                make_pair<CvPoint, int>(cornerContainer.at(i), i));
        ...
        pair<int, setStruct> pairs = make_pair<int, setStruct > (1,
                structuredObject);
    ...
    for (i = 0; i < cornerContainer.size(); i++) {
        for (j = 0; j < cornerContainer.size(); j++) {
            ...
            //	continue;
            if (isOnTheSameline(cornerContainer.at(i), cornerContainer.at(j),
                    lineContainer)) {
                printf("Merge called for\n");
                mergeSet(i, j, containers, cornerContainer.at(i),
                        cornerContainer.at(j));
                //break;
            }
        }
    ...
    for (i = 0; i < cornerContainer.size(); i++) {
        ...
        if (i != containers[i].first) {
            superContainer.at(containers[i].first).push_back(
                    make_pair<CvPoint, int>(cornerContainer.at(i), i));
        }
    }
    for (i = 0; i < superContainer.size(); i++) {
        ...
        if (superContainer.at(i).size() >= 4) {
            ...
            for (int j = 0; j < superContainer.at(i).size(); j++) {
                ...
                graph[superContainer.at(i).at(j).second] =
                        containers[superContainer.at(i).at(j).second].second.second.Node;
                sort(graph[superContainer.at(i).at(j).second].begin(),
                        graph[superContainer.at(i).at(j).second].end(),
                        sortOutAnother);
                for (int k = 0;
                        k < graph[superContainer.at(i).at(j).second].size();
                        k++) {
                    if (visited[graph[superContainer.at(i).at(j).second][k].first][graph[superContainer.at(
                            i).at(j).second][k].second] == 0) {
                        ...
                        visited[graph[superContainer.at(i).at(j).second][k].first][graph[superContainer.at(
                                i).at(j).second][k].second] = 1;
                        visited[graph[superContainer.at(i).at(j).second][k].second][graph[superContainer.at(
                                i).at(j).second][k].first] = 1;
                    }
                }
            }
            ...
            for (int l1 = 0; l1 < indexHoldering.size(); l1++) {
                ...
                if (indexHoldering.at(l1).size() != 0) {
                    for (int l2 = 0; l2 < indexHoldering.at(l1).size() - 1;
                            l2++) {
                        ...
                        tmpContainerVec.push_back(
                                cornerContainer.at(
                                indexHoldering.at(l1).at(l2)));
                    }
                    ...
                    if ((sizeVal - 2) == tmpContainerVec.size()
                            && counterVal == 1) {
                        // for(int l2 = 0; l2 < indexHoldering.at(l1).size()-1; l2++)
                        //cvDrawLine(img,cornerContainer[indexHoldering.at(l1).at(l2)],cornerContainer[indexHoldering.at(l1).at(l2+1)],cvScalar(255,0,0),2);
                        sort(tmpContainerVec.begin(), tmpContainerVec.end(),
                                sortOutPoints);
                        if (pX.x > tmpContainerVec.at(0).x)
                            pX.x = tmpContainerVec.at(0).x;
                        if (pY.x
                                < tmpContainerVec.at(tmpContainerVec.size() - 1).x)
                            pY.x = tmpContainerVec.at(
                                tmpContainerVec.size() - 1).x;
                        sort(tmpContainerVec.begin(), tmpContainerVec.end(),
                                sortOutPointsY);
                        if (pX.y > tmpContainerVec.at(0).y)
                            pX.y = tmpContainerVec.at(0).y;
                        if (pY.y
                                < tmpContainerVec.at(tmpContainerVec.size() - 1).y)
                            pY.y = tmpContainerVec.at(
                                tmpContainerVec.size() - 1).y;
            ...
            cvPoints[1].x = superContainer.at(i).at(
                    superContainer.at(i).size() - 1).first.x;
            //sort(superContainer.at(i).begin(), superContainer.at(i).end()sortOutPointY);
            cvPoints[0].y = superContainer.at(i).at(0).first.y;
            cvPoints[1].y = superContainer.at(i).at(
                    superContainer.at(i).size() - 1).first.y;
            ...
            superContainer.at(i).push_back(
                    make_pair<CvPoint, int>(cvPoints[0], 0));
            superContainer.at(i).push_back(
                    make_pair<CvPoint, int>(cvPoints[1], 1));
