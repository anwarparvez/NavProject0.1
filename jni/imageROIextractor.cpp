...
#define TRIVA(a,it)\
	for(vector<pairing>::iterator it = a.begin(); it != a.end();it++)
void initGraph(){
...
bool isOnTheSameline(CvPoint p1, CvPoint p2, vector<CvPoint*> &lineContainer){
 ...
 bool sortOutPointX(const pair<CvPoint,int> p1,const pair<CvPoint,int> p2){
	 return p1.first.x == p2.first.x ? p1.first.y < p2.first.y : p1.first.x < p2.first.x ; 
 }
 bool sortOutPointY(const pair<CvPoint,int> p1, const pair<CvPoint,int> p2){
