...
bool isVerticalline(vector<CvPoint*> verticalLineholder, CvPoint points) {
	...
	for (int i = 0; i < verticalLineholder.size(); i++) {
		...
		if (getLineLenght(pointsget[0], pointsget[1]))
			continue;
		double val1 = (double) ((pointsget[0].x - points.x)
				* (pointsget[0].x - points.x)
				+ (pointsget[0].y - points.y) * (pointsget[0].y - points.y));
		double val2 = (double) ((pointsget[1].x - points.x)
				* (pointsget[1].x - points.x)
				+ (pointsget[1].y - points.y) * (pointsget[1].y - points.y));
	...
	for (int layer = 0; layer < srcMatrix.rows / 2; ++layer) { //one layer is a complete circle
		...
		for (int i = first; i < last || i < lastColsDst; ++i) {
			...
			if (points2.y > 0)			// checks for whether height is positive
				dstMatrix.at < cv::Vec3b > (points2) = srcMatrix.at < cv::Vec3b
						> (points1);
			if (i < lastRows) {	// checks whether the last row of src matrix reaches.
				//left -> top
				points2.x = last;
				points2.y = i;
				dstMatrix.at < cv::Vec3b > (points1) = srcMatrix.at < cv::Vec3b
						> (points2);
			}
...
void imageRotationMat(Mat &image, double angle) {
	...
	int newHeight = image.rows * fabs(cos(angle))
			+ fabs(sin(angle)) * image.cols;
	int newWidth = image.rows * fabs(sin(angle))
			+ fabs(cos(angle)) * image.cols;
	...
	for (int i = 0; i < image.rows; i++) {
		for (int j = 0; j < image.cols; j++) {
			...
			if (tempIndex.at<double>(0, 0) - floor(tempIndex.at<double>(0, 0))
					< 0.5)
				tempIndex.at<double>(0, 0) = floor(tempIndex.at<double>(0, 0));
			else
				tempIndex.at<double>(0, 0) = ceil(tempIndex.at<double>(0, 0));

			if (tempIndex.at<double>(1, 0) - floor(tempIndex.at<double>(1, 0))
					< 0.5)
				tempIndex.at<double>(1, 0) = floor(tempIndex.at<double>(1, 0));
			else
				tempIndex.at<double>(1, 0) = ceil(tempIndex.at<double>(1, 0));
			resultantMat.at < cv::Vec3b
					> ((int) (tempIndex.at<double>(0, 0) + resultHeight), (int) (tempIndex.at<
							double>(1, 0) + resultWidth)) = image.at < cv::Vec3b
					> (i, j);
		}
...
vector<CvPoint> getFilteredpoints(CvMat* mat,
...
int histogramRGB(void) {
	...
	calcHist(&src, 1, channels, Mat(), r_hist, 1, &histSize, &histRange,
			uniform, accumulate);
	...
	normalize(r_hist, r_hist, (double) 0, (double) histImage.rows, NORM_MINMAX,
			-1, MatND());
...
void cvCreateHistogram(IplImage *image, IplImage *histImage) {
	...
	CvHistogram* histogram = cvCreateHist(1, &histSize, CV_HIST_ARRAY, ranges,
			1);
	cvCalcHist(&image, histogram, 0, NULL);
	cvGetMinMaxHistValue(histogram, 0, &maxVal, 0, 0);
	cvScale(histogram->bins, histogram->bins,
			((float) histImage->height) / maxVal, 0);
	w_Scale = ((float) histImage->width) / histSize;

	for (int i = 0; i < histSize; i++) {

		cvRectangle(histImage, cvPoint((int) i * w_Scale, histImage->height),
				cvPoint((int) (i + 1) * w_Scale,
						histImage->height
								- cvRound(cvGetReal1D(histogram->bins, i))),
				cvScalar(0), -1, 8, 0);
	}
